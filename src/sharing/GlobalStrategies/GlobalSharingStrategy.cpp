...
GlobalSharingStrategy::initMpiVariables()
{
        ...
        if (Painless::mpi_rank == MY_MPI_ROOT) {
                this->receivedFinalResultRoot.resize(Painless::mpi_world_size - 1);
                this->recv_end_requests.resize(Painless::mpi_world_size - 1);

                for (int i = 0; i < Painless::mpi_world_size - 1; i++) {
...
void
GlobalSharingStrategy::joinProcess(int winnerRank, Painless::SatResult res, const std::vector<int>& model)
{
        //=================================================================
        // Temporary
        // ---------
        ...
        if (MY_MPI_ROOT != Painless::mpi_rank) {
                // Non-root processes
