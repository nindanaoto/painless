...
AllGatherSharing::doSharing()
{
        ...
        TESTRUNMPI(MPI_Comm_split(MPI_COMM_WORLD, this->color, Painless::mpi_rank, &yes_comm));

        if (this->color == MPI_UNDEFINED) {
                LOG2("[Allgather] is not willing to share (%d)", Painless::mpi_rank);
        } else // do this only if member of yes_comm
        {
                ...
                LOGDEBUG3("[Allgather] before allgather", Painless::mpi_rank);
