...
class SharingEntity : public std::enable_shared_from_this<SharingEntity>
{
  public:
         ...
         */
        virtual bool importClause(const Painless::ClauseExchangePtr& clause) = 0;
         ...
         * Derived classes should ensure thread-safety in their implementation.
         */
        virtual void importClauses(const std::vector<Painless::ClauseExchangePtr>& v_clauses) = 0;
  ...
  protected:
         ...
         */
        virtual bool exportClauseToClient(const Painless::ClauseExchangePtr& clause, std::shared_ptr<SharingEntity> client)
        {
         ...
         */
        bool exportClause(const Painless::ClauseExchangePtr& clause)
        {
                std::shared_lock<std::shared_mutex> lock(m_clientsMutex);
                bool exported = false;
         ...
         * Subclasses can customize the behavior of clause export by overriding the exportClauseToClient method.
         */
        void exportClauses(const std::vector<Painless::ClauseExchangePtr>& clauses)
        {
                std::shared_lock<std::shared_mutex> lock(m_clientsMutex);
                for (const auto& weakClient : m_clients) {
                        if (auto client = weakClient.lock()) {
                                for (const Painless::ClauseExchangePtr& clause : clauses) {
