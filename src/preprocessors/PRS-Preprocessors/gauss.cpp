...
bool
cmpxorgate(xorgate a, xorgate b)
{
        return a.sz > b.sz;
}
...
bool
preprocess::preprocess_gauss()
{
        int nxors = search_xors();
        LOG2("[PRS %d]  [GE] XORs: %d (time: 0.00)", this->getSolverId(), nxors);
        if (!nxors)
                return true;
        int nvarscc = ecc_var();
        LOG2("[PRS %d]  [GE] VAR SCC: %d", this->getSolverId(), nvarscc);
        int nxorscc = ecc_xor();
        LOG2("[PRS %d]  [GE] XOR SCCs: %d (time: 0.00)", this->getSolverId(), nxorscc);
        int res = gauss_elimination();
        LOG2("[PRS %d]  [GE] unary xor: %d, bin xor: %d, bin added", this->getSolverId(), gauss_eli_unit, gauss_eli_binary);
        if (!res) {
                LOG2("[PRS %d] [GE] UNSAT", this->getSolverId());
        }
        xors.clear();
        scc_id.clear();
        for (int i = 0; i < scc.size(); i++)
