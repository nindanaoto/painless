...
YalSat::YalSat(int _id, unsigned long flipsLimit, unsigned long maxNoise)
        ...
        , m_maxNoise(maxNoise)
        , Painless::LocalSearchInterface(_id, LocalSearchType::YALSAT)
{
        // Yals * yals_new ();
...
YalSat::solve(const std::vector<int>& cube)
{
        ...
        if (static_cast<int>(Painless::SatResult::SAT) != res) {
                return Painless::SatResult::UNKNOWN;
        }

        return Painless::SatResult::SAT;
}
...
void
YalSat::diversify(const Painless::SeedGenerator& getSeed)
{
